...
class Patronus:
    ...
    def _request(self, path: str, body: bytes | None, headers: dict[str, str], deadline: float):
        request = Request(f"{self.base_url}{path}", data=body, headers={"Accept": "application/json", "Authorization": f"Bearer {self.api_key}", "User-Agent": "patronus-api-client-python/0.1.2", **headers}, method="POST" if body is not None else "GET")
        try:
