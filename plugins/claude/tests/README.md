...
- Host tests run the installed CLI with deterministic hook decisions and the production mapper. They prove the exact `UserPromptSubmit.prompt` value is visible before the first model request; Bash/Read replacement; plain, JSON-looking, and mixed-media MCP text visibility; exact failure-result text visibility; exact placeholder transport and authoritative session identity; failure quarantine with PostToolBatch; resume blocking; and plugin discovery using the shipped manifest/config files with a fixture transport stub.
- Runtime tests run the installed CLI with the **built shared controller/broker and installed local scanner**. They prove pending Bash replacement; genuine MCP error quarantine plus PostToolBatch/resume blocking; ordinary/placeholder request denial without PostToolUseFailure; and quarantine of successful generic MCP responses carrying hidden sibling metadata.
- Negative controls establish the limits: PostToolUseFailure alone ignores `continue:false`; MCP structured metadata survives in the transcript. The production timeout test runs the shipped bundle directly with a deterministic delayed local scanner peer. It proves the controller's durable pending-result marker makes PostToolBatch stop after a launched PostToolUse hook times out. A hook that never launches remains outside this proof.

Ordinary `PreToolUse` requests are outside the runtime text contract and are not scanned. Patronus' own placeholder tools are still intercepted before execution. Successful production `PostToolUse` replacement uses `updatedToolOutput` matching the hook-visible result shape. Plain strings are replaced as strings; Bash/Read structures retain safe native shapes; ordered text blocks are replaced without including adjacent media in scanner input. PostToolBatch stops with `{"continue":false,"stopReason":"SAFE_TEXT"}` after durable quarantine. Claude PostToolUse `decision:"block"` does not implement replacement.
