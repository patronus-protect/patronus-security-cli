...
Use the plugin's `patronus_check_result` tool with a runtime receipt's `scan_id` to check status. Use `patronus_read_redacted` with `scan_id` for a dangerous runtime result, or with `file_id` for a static finding whose scan reports redaction is available. Never pass a static `file_id` as `scan_id`. A still-pending result can be checked again later; do not claim completion before its status is known. Failed, unavailable or denied results are not approval to recover the original by another tool.
