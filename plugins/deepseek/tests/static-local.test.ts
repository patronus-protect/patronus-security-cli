...
if (!executable) throw new Error('Set PATRONUS_TEST_SCANNER to the trusted installed Ark 0.1.6 CLI.')
const signal = () => new AbortController().signal

it('scans clean file/directory/repo and the known dangerous document with installed Ark 0.1.6, without file bytes in model requests', async () => {
  const version = await promisify(execFile)(executable, ['version'])
  expect(version.stdout).toContain('patronus-ark 0.1.6')
