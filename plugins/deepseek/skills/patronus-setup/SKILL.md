---
name: patronus-setup
description: Set up or diagnose Patronus Security CLI authentication, processing mode, local models and the global DeepSeek integration when the user requests installation or setup.
---
...
After setup, inspect `patronus-security-scanner onboarding --status --format json` and `patronus-security-scanner integration deepseek status --format json`. Start a new host session and verify actual hooks are active; a saved benchmark or installation record alone does not prove host protection.
...
For updates use `patronus-security-scanner maintenance update` and `patronus-security-scanner integration deepseek update`. Public release installation requires the release artifacts to have been published. Use the CLI help for explicit source overrides during development.
